...
unsigned long _hash(const char *file_name) {
	unsigned long id = 0;
...
int ZPackFileNT::getNodeIndex(unsigned long id) {								//二分法找到指定的索引
	int i;
	for(i=0;i<header.count;++i)
	{
		if (id == index_list[i].id) break;
...
char *ZPackFileNT::getData(const char *name) {											//获取指定的文件数据
	unsigned long id = _hash(name);
	return getData(id);
}
