...
void CClientManager::BeginToSend()
{
			...
			i ++ )
	{
		...
		static const DWORD dwEOSFlag = 0xABCD0000; // End of stream
		m_clientContext[ulnID].pWriteBuffer->AddData( reinterpret_cast<const char*>( &dwEOSFlag ), sizeof(DWORD) );

#endif  //NETWORK_DEBUG
		//]
		
		const BYTE *pPackData = m_clientContext[ulnID].pWriteBuffer->GetBuffer();
		size_t used = m_clientContext[ulnID].pWriteBuffer->GetUsed();
		//const size_t used = m_clientContext[ulnID].pWriteBuffer->GetUsed();

		//[
#ifdef NETWORK_DEBUG	//add by spe
		if (used <= 4)
		{
			used = 0;
		}
...
unsigned long AddClient( 
		OnlineGameLib::Win32::CSocketServer::Socket *pSocket )
{
	return g_theClientManager.AddClient( pSocket );
}
